      ...
      {page === 'settings' && <>
        ...
        <div className="settings-stack">
          ...
          <section className="panel settings-card" aria-labelledby="proxy-settings-title">
            ...
            <div className="setting-row port-setting"><label htmlFor="mixed-port"><strong>本地代理端口</strong>{connected && <small>保存后将按当前网络设置重新连接</small>}</label><div className="port-editor"><div className="port-field"><span className="port-address">127.0.0.1:</span><input id="mixed-port" type="number" min="1024" max="65535" step="1" inputMode="numeric" value={portDraft} disabled={busy} onChange={event => setPortDraft(event.target.value)} /></div><button className="secondary-button" type="button" disabled={busy || portDraft === String(state.port)} onClick={() => void saveMixedPort()}>保存</button></div></div>
          </section>
