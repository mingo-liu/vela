...
- 系统代理只修改当前默认网络接口对应的服务，保存原有 HTTP、HTTPS、SOCKS、PAC 与自动发现设置。关闭时逐字段确认仍指向 Vela 才恢复，避免覆盖 Clash Verge 等其他应用后续的修改。切换 Wi-Fi 或有线服务后，需要先关闭再开启系统代理以接管新服务；需要认证的已有代理不会被接管。修改网络设置需要当前用户具备 macOS 管理员权限；授权失败时，本地代理仍可使用。
